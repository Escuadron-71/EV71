...
- **Focus mode:** boton "Enfoque" en el HUD alterna la clase `planner-focus` en `document.body` (efimero, no persistido): oculta header/footer/page-head/sidebars y maximiza mapa a `100vh`; boton flotante "✕ Salir" para volver; al alternar se llama `map.invalidateSize()`. Reglas en `_planificador.scss`.
