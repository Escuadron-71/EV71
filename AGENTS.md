...
- **Fase 3 (B3) — Bases aereas:** herramienta "Bases" (cuarto boton en la barra persistente, junto a Navegacion/Amenazas/Referencia): alterna la capa de aerodromos del teatro activo. 100 aerodromos (Caucaso 21 + Siria 79) en `src/lib/planner/plugins/airfields/airfields.ts`, generados por el script build-time `backend/scripts/airfields-extract/extract.mjs` (gitignored) que lee los dumps de pydcs y convierte coordenadas con `proj4` (**solo en build; `proj4` NO es dependencia runtime**). Marcadores fijos `divIcon` `planner-airfield-marker` (rombo dorado); popup `planner-airfield-popup` con nombre, tag civil, coordenadas, TACAN, tabla ATC (HF/VHF-low/VHF-high/UHF en MHz) y pistas con cabeceras y rumbos. Bloque "Bases aereas" en el panel: busqueda por nombre + lista + centrar (pan/fit y abre el popup). **Limitacion conocida:** los dumps de pydcs no incluyen frecuencias ILS ni canales TACAN (campo `tacan` null → UI muestra "—"); el schema conserva los campos para enriquecerlos a futuro. Estado persistido `airfieldsVisible` en `ev71-planner-state` (validacion backward-compat).
- **Fase 3 (B7) — Kneeboard:** bloque "Kneeboard" en el panel: boton "Generar kneeboard" abre el modal `KneeboardExport.tsx` (`src/islands/planner/widgets/`) que renderiza tarjetas **512x512 PNG via Canvas API** (sin librerias externas): RESUMEN (teatro, aeronave, velocidad, navegacion, distancia, ETE, despegue/TOT), WAYPOINTS (+BR desde bullseye), TRAMOS (rumbo, distancia, ETE, Acum, ETA), FRECUENCIAS, BREVITY y AMENAZAS. Descarga individual o en lote. Logica pura SSR-safe en `src/lib/planner/kneeboard.ts` (`buildKneeboardCards`) con constantes de color de marca (`KNEEDARK`, `KNEE_PANEL`, `KNEE_GOLD`, `KNEE_TEXT`, `KNEE_DIM`).
